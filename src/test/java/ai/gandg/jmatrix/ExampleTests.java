...
public class ExampleTests {
  ...
  @Test public void solveLeastSquaresFR() {
    ...
    Matrix b = Matrix.create(4., 4., 9., -5.).T();
    // Matrix x = A.solve(b);
    Matrix[] QR = A.QR(false); // without computing Q, size(R): 4x4
  ...
  @Test public void solveLeastSquaresFCR() {
    ...
    Matrix b = Matrix.create(6., 5., 7., 10.).T();
    // Matrix x = A.solve(b);
    Matrix[] QR = A.QR(false); // without computing Q, size(R): 4x2
    Matrix x = QR[1].backsU(QR[1].T().backsL(A.T().mul(b)));
  ...
  @Test public void solveLeastSquaresFRR() {
    ...
    // Matrix x = A.solve(b);
    Matrix[] QR = A.T().reducedQR(); // size(Q): 4x2, size(R): 2x2
  ...
  // @Test public void solveLeastSquaresLRRTest() {
  //   // A has low (row) rank.
