...
public abstract class Matrix {
  ...
  /**
   * Creates a dense matrix with a single row (row vector) by the provided values.
   *
   * @param data elements of the matrix
   * @return matrix which encapsulates <code>data</code> (not copied)
   */
  ...
  public static Matrix create(int rows, int cols) {
    ...
    if (cols > 0 && (rows == 0 || rows > cols)) {
      return Matrix.of(new double[cols][rows]).T();
    }
    return Matrix.of(new double[rows][cols]);
  }
   ...
   * with L being the lower triangular matrix <code>this</code>.
   *
   * Matrix <code>this</code> should be non-singular.
   ...
   * with U being the upper triangular matrix <code>this</code>.
   *
   * Matrix <code>this</code> should be non-singular.
  ...
  // TODO: this code needs P'QR decomposition (QR with column pivoting)
  // to handle rank deficient problems...
